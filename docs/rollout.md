...
2. 在目标仓添加事件 intake 和对应的 `*-codex-run.yml`，配置 `shadow` Environment、`OPENAI_API_KEY`、控制仓读取 token、模型变量和只读 `GITHUB_TOKEN`；intake 只 dispatch，不使用 `actions/checkout`，也不接触 OpenAI/Telegram Secret。
